...
def screen_gene(gene_aminoacids, codons, out_file, name): # Screen the given gene after the defined stretches
	...
	while j < number_of_codons - args.length: # j value is the index for the beginning of the stretch
		...
		if CSC_sum >= args.min and CSC_sum <= args.max and no_end == False: # Test if the stretch corresponds to the wanted values
			out_file.write("\n" + str.split(name, ' ')[0] + ";" + str(number_of_codons) + ";" + str(j) + ";") # Write variables
			no_end = True
		
		j += 1 # Go to the next codon

		codon = get_codon(gene_aminoacids, j, name, number_of_codons) # Define the codon that will be excluded in the next stretch
		CSC_sum -= codons[codon] # Deduct its value
		codon = get_codon(gene_aminoacids, j + args.length, name, number_of_codons) # Define the codon that will me added in the next stretch
		if codon == False: # If there is no codon to be added
			if no_end == True:
				out_file.write(str(j - 1 + args.length)) # Write the last codon position
				no_end = False
			break
		...
		if (CSC_sum <= args.min or CSC_sum >= args.max) and no_end == True: # If it does not correspond to the wanted values anymore
			out_file.write(str(j - 1 + args.length)) # Write the end position
			no_end = False
...
def main():
	...
	out_file.write("gene;gene length (in codons);start position (in codons);end position (in codons)")
