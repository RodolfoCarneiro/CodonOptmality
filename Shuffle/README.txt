...
This algorithm writes a fasta-format file, with sequences the same size as the base fasta file, but with random codons propotional to the base file or to a external csv file.

Usage:
shuffler.py [-h] -F <example.fasta> -O <output.csv> -E <frequency_example.csv> [--keep_seq]
...
-E, --external (optional): CSV-format file containing the frequency values for every codon. If absent, the frequency will be calculated based on the base fasta file.
--keep_seq (optional): Use in order to keep the same aminoacid sequence as the base fasta file.
